...
PCLASS = 'F' # processing type ('M' manually assisted,  'F' for "fully automatic", or 'R' for "relevance feedback")
TASK = 'M' ##Task type ('M' Main task, 'P' Progress subtask)
NOVELTY= 'C' ##('N' Novelty run, 'C' Common run - not focused on unique shots)
ALGO = 'V'  ##Indicates if the run is based on a concept-based ('C') approach (vs other techniques ('V') such as embeddings, etc)
PID = 'VIREO' # short participant ID
PRIORITY = 1
TOPK = 1000
DESC = 'place holder'
XML_HEAD = '<!DOCTYPE videoAdhocSearchResults SYSTEM "https://www-nlpir.nist.gov/projects/tv2023/dtds/videoAdhocSearchResults.dtd">'

ETIME = 1 # the time in seconds from the time the system is presented with the topic until the searching on that topic ended
...
def process(options, collection, input_txt_file):
    ...
    for line in data:
        ...
        outlines=[]
        outfilename =  input_txt_file + '.'+tNum
    ...
    xml_file.append('<videoAdhocSearchRunResult trType="%s" class="%s" task="%s" novelty="%s" algo="%s" pid="%s" priority="%s" desc="%s">' % (trtype, pclass, task,novelty, algo, pid, priority, desc))
...
def main(argv=None):
    ...
    parser.add_option("--task", default=TASK, type="string", help="description of this run (default: %s)" % DESC)
    parser.add_option("--novelty", default=NOVELTY, type="string", help="description of this run (default: %s)" % DESC)
    parser.add_option("--algo", default=ALGO, type="string", help="description of this run (default: %s)" % DESC)
